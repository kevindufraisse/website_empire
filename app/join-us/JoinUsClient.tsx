...
const KEVIN_IMG = '/founders/kevin.png'
...
export default function JoinUsClient() {
  ...
  const accent = {
    ...
    bg: autopilot ? 'bg-autopilot' : 'bg-empire',
    bgSoft10: autopilot ? 'bg-autopilot/10' : 'bg-empire/10',
    hoverText: autopilot ? 'hover:text-autopilot' : 'hover:text-empire',
    hoverTextSoft: autopilot ? 'hover:text-autopilot/90' : 'hover:text-empire/90',
    hoverBorderSoft: autopilot ? 'hover:border-autopilot/30' : 'hover:border-empire/30',
    glowRadial: autopilot
      ? 'bg-[radial-gradient(ellipse_100%_50%_at_50%_-10%,rgba(212,165,116,0.10),transparent)]'
      : 'bg-[radial-gradient(ellipse_100%_50%_at_50%_-10%,rgb(var(--empire-rgb)_/_0.08),transparent)]',
    heroSpan: autopilot ? 'text-autopilot' : 'text-empire',
    formBorder: autopilot ? 'border-autopilot/30' : 'border-empire/30',
    formShadow: autopilot
      ? 'shadow-[0_0_60px_-20px_rgba(212,165,116,0.35)]'
      : 'shadow-[0_0_60px_-20px_rgb(var(--empire-rgb)_/_0.30)]',
    formBar: autopilot ? 'bg-autopilot' : 'bg-empire',
  }
  ...
  return (
    <main className="min-h-screen bg-black text-white overflow-x-hidden">
      <div className="fixed inset-0 pointer-events-none">
        <div className={`absolute inset-0 ${accent.glowRadial}`} />
      </div>

      <div className="relative z-10 flex flex-col min-h-screen">
        <div className="border-b border-white/5">
          <div className="flex items-center justify-between max-w-5xl mx-auto w-full px-4 sm:px-6 py-4 gap-3">
            <a href="/" className={`text-white font-bold text-base tracking-tight ${accent.hoverText} transition-colors shrink-0`}>
              Empire Internet
            </a>
        ...
        <div className="flex-1 w-full max-w-5xl mx-auto px-4 sm:px-6 py-8 md:py-14 grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-start">
          ...
          <div className="lg:sticky lg:top-8 order-2 lg:order-1">
            <h1 className="text-3xl sm:text-4xl font-extrabold leading-tight mb-4">
              {headline}{' '}
              <span className={accent.heroSpan}>{headlineAccent}</span>
            </h1>
            ...
            <div className="flex items-center gap-2 flex-wrap mb-6">
              ...
              {platforms.map((p) => (
                <div
                  key={p.name}
                  title={p.name}
                  className={`w-8 h-8 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center ${accent.hoverBorderSoft} transition-all`}
                >
